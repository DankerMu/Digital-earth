...
class IngestScheduler:
    def __init__(
        ...
        *,
        ...
        on_success: Optional[Callable[[IngestRun], Awaitable[None]]] = None,
        max_retries: int = 3,
            ...
            try:
                while True:
                    ...
                    if self._on_success is not None:
                        try:
                            await self._on_success(updated)
                        except asyncio.CancelledError:
                            raise
                        except Exception as exc:  # noqa: BLE001
                            logger.error(
                                "ingest_on_success_failed",
                                extra={"run_id": updated.run_id, "error": str(exc)},
                            )
                    return updated
            except asyncio.CancelledError:
                updated = self._run_store.update_run(
