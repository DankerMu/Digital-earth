...
def test_copernicus_item_url_negative_coords() -> None:
    ...
    assert client.item_url(tile).endswith("/items/Copernicus_DSM_COG_10_S08_00_W010_00.json")
...
def test_demgrid_sample_nan_handling() -> None:
    ...
    assert grid.sample(0.0, 0.0, method="nearest", fill_value=123.0) == pytest.approx(123.0)
    assert grid.sample(0.5, 0.5, method="bilinear", fill_value=123.0) == pytest.approx(123.0)
...
def test_demgrid_sample_grid() -> None:
    ...
    out = grid.sample_grid(GeoRect(west=0.0, south=0.0, east=1.0, north=1.0), grid_size=3)
...
def test_dem_mosaic_neighbor_and_scan_fallback() -> None:
    ...
    with pytest.raises(ValueError, match="grid_size must be"):
        mosaic.sample_grid(GeoRect(west=0.0, south=0.0, east=1.0, north=1.0), grid_size=1)


def test_copernicus_download_geotiff_with_fake_httpx(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
