...
describe('riskApi', () => {
  ...
  it('splits dateline-crossing bboxes when evaluating risk by bbox', async () => {
    const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      const url = typeof input === 'string' ? input : input.toString();
      if (url === 'http://api.test/api/v1/risk/evaluate') {
        expect(init?.method).toBe('POST');
        ...
        if (body.bbox?.join(',') === '170,10,180,20') {
          return jsonResponse({
            summary: {
              total: 1,
              duration_ms: 1,
            ...
            results: [{ poi_id: 2, level: 4, score: 0.9, factors: [], reasons: [] }],
          });
        }

        if (body.bbox?.join(',') === '-180,10,-170,20') {
          return jsonResponse({
            summary: {
              total: 1,
              duration_ms: 1,
            ...
            results: [{ poi_id: 1, level: 3, score: 0.8, factors: [], reasons: [] }],
          });
        }
      }
      return new Response('Not Found', { status: 404 });
    ...
    expect(result.results.map((item) => item.poi_id)).toEqual([1, 2]);
    expect(fetchMock).toHaveBeenCalledTimes(2);
  });
